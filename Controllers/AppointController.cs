...
namespace AutomobileServiceCenter_MasterDetailsInAPI.Controllers
{
    ...
    public class AppointController : ControllerBase
    {
        ...
        {
            var appointments = _dbContext.AppointMasters.Include(a => a.OrderDetail).ToList();
        ...
        {
            var appointment = _dbContext.AppointMasters.Include(a => a.OrderDetail).FirstOrDefault(a => a.AppointId == appointId);
            if (appointment == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> PostAppointment()
        {
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(appointDetailJson))
                {
                    ...
                    appointment.OrderDetail.AddRange(appointDetailList);
                }
        ...
        {
            var appointment = _dbContext.AppointMasters.Include(a => a.OrderDetail).FirstOrDefault(a => a.AppointId == id);
            if (appointment == null)
            {
                return NotFound();
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(appointDetailJson))
                {
                    ...
                    {
                        var existingAppointDetail = appointment.OrderDetail.FirstOrDefault(ad => ad.AppointDetailId == updatedAppointDetail.AppointDetailId);
                        ...
                        {
                            appointment.OrderDetail.Add(new AppointDetail
                            {
